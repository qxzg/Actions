...
get_gfwlist() {
	...
	# 2. merge
	cat ${CURR_PATH}/gfwlist_download.conf ${CURR_PATH}/gfwlist_fancyss.conf | grep -Ev "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | sed "s/^/server=&\/./g" | sed "s/$/\/0.0.0.0#7913/g" >${CURR_PATH}/gfwlist_merge.conf
